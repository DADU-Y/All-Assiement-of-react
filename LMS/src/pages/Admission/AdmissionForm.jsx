...
export default function AdmissionForm(){
  ...
  useEffect(()=>{
    if(id){
      (async ()=>{
        const d = await getDoc(doc(db,'Admission',id))
  ...
  const onSubmit = async data => {
    try {
      if(id){
        await setDoc(doc(db,'Admission',id), data, { merge: true })
      } else {
        await addDoc(collection(db,'Admission'), { ...data, createdAt: new Date().toISOString() })
      }
      navigate('/Admission')
  ...
  return (
    <div className="container">
      <h4>{id ? 'Edit Admission' : 'Add Admission'}</h4>
