...
export default function FeeForm(){
  ...
  useEffect(()=>{
    if(id){
      (async ()=>{
        const d = await getDoc(doc(db,'Fee',id))
  ...
  const onSubmit = async data => {
    try {
      if(id){
        await setDoc(doc(db,'Fee',id), data, { merge: true })
      } else {
        await addDoc(collection(db,'Fee'), { ...data, createdAt: new Date().toISOString() })
      }
      navigate('/students')
